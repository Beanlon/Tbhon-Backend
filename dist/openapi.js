...
exports.openApiSpec = {
    ...
    info: {
        ...
        version: "1.0.0",
        description: "REST API for the Tbhon TB screening app.\n\n" +
    ...
    components: {
        securitySchemes: {
            bearerAuth: {
                ...
                scheme: "bearer",
                bearerFormat: "JWT",
                description: "JWT from POST /auth/login or /auth/register. In Authorize, paste the token only (Swagger adds `Bearer `). Do not use the IoT shared key here.",
            },
            iotKey: {
                type: "apiKey",
                in: "header",
            ...
            CompleteScreeningBody: {
                ...
                properties: {
                    ...
                    checklist: {
                        ...
                        example: '{"version":1,"items":[{"id":"symptom_persistent_cough","label":"Persistent cough","value":true}]}',
                    },
                    audioUris: {
                        type: "array",
                        items: { type: "string" },
            ...
            IotDeviceCommandBody: {
                ...
                properties: {
                    command: {
                        ...
                        example: "audio",
                        description: "`audio` starts recording. `audio upload` asks the firmware to stop and upload, and is only accepted after the 3 second minimum has passed.",
                    },
                },
            },
        },
    ...
    paths: {
        ...
        "/screenings/{sessionId}/cough-recordings/{recordingId}/raw": {
            post: {
                ...
                description: "Used by the mobile app immediately after `POST /screenings` so the original `.wav`/`.m4a` is persisted server-side and any other phone on this account can play it.",
                security: [{ bearerAuth: [] }],
                parameters: [
        ...
        "/iot/hello": {
            post: {
                ...
                description: "Use this from an ESP32/microcontroller to verify the backend URL and IoT key before uploading real audio/image bytes. Requires only `X-IoT-Key`.",
                security: [{ iotKey: [] }],
                requestBody: {
                    required: false,
                    content: {
        ...
        "/iot/device-command": {
            get: {
                ...
                description: "ESP32-friendly polling endpoint. Returns plain text body: `image`, `audio`, `audio upload`, or empty string when no command is queued. By default it consumes the queued command on read.",
                security: [{ iotKey: [] }],
                parameters: [
                    {
                        in: "query",
            ...
            post: {
                ...
                description: "Queues a single command that your microcontroller can fetch from `GET /iot/device-command`. Allowed values are `image`, `audio`, and `audio upload`. `audio upload` is accepted only after the device has received `audio` and the 3 second minimum has passed.",
                security: [{ iotKey: [] }],
                requestBody: {
                    required: true,
                    content: {
        ...
        "/iot/cough-recordings": {
            post: {
                ...
                description: "Accepts multipart (`file=`) **or** JSON with `fileBase64`. Requires `X-IoT-Key` header. If `sessionId` is omitted, a new screening session is created automatically for the given `userId`.",
                security: [{ iotKey: [] }],
                requestBody: {
                    required: true,
                    content: {
        ...
        "/iot/sputum-images": {
            post: {
                ...
                description: "Accepts multipart (`file=`) **or** JSON with `fileBase64`. Requires `X-IoT-Key`. If a sputum image already exists for the session, it is replaced. The response includes an IoT-key protected `fileUrl` for downloading the bytes without a mobile JWT.",
                security: [{ iotKey: [] }],
                requestBody: {
                    required: true,
                    content: {
        ...
        "/iot/sputum-images/{sessionId}/file": {
            get: {
                ...
                description: "Use this for IoT/debug tooling with `X-IoT-Key`. Mobile app media routes still use the user's bearer JWT under `/screenings/*`.",
                security: [{ iotKey: [] }],
                parameters: [
                    { $ref: "#/components/parameters/SessionIdPath" },
